...
func Test_tokenCache_Save(t *testing.T) {
	client, miniredis := mockredis.Connection()
	...
	tests := []struct {
	...
	} {
		{
			...
			args: args{
				...
				token: &model.Token{
					UniqueToken: "R1bAeiPkJOOA8MqJVgMieGBVd1QcqR65Av7hKpIZTHbe6QrFdzIii6Q7Hsp-YSsEukn7iWRt",
					TimeAt: "1594379699",
				},
			},
			expecting: func() {
				miniredis.CheckGet(t, "auth:token:1", `{"unique_token":"R1bAeiPkJOOA8MqJVgMieGBVd1QcqR65Av7hKpIZTHbe6QrFdzIii6Q7Hsp-YSsEukn7iWRt","time_at":"1594379699"}`)
			},
		},
		{
			name: "redis token expire",
			args: args{
				user: &model.User{
					ID:        "1",
					Email:     "test@email.com",
			...
			expecting: func() {
				miniredis.FastForward(time.Second * 60 * 60 * 25)
				if miniredis.Exists("auth:token:1") {
					t.Error("This should not be existed anymore")
				}
			},
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := uc.SaveToken(tt.args.token, tt.args.user); (err != nil) != tt.wantErr {
				t.Errorf("tokenCache.Save() error = %v, wantErr %v", err, tt.wantErr)
			}
			tt.expecting()
		})
	}
...
func Test_tokenCache_Get(t *testing.T) {
	client, miniredis := mockredis.Connection()
	...
	tests := [] struct {
		...
		},
		{
			...
			initMock: func() {
				_ = miniredis.Set("user:1", `{"id":"1","username":"test","email":"test@email.com","created_at":1578831132,"last_login":1578831132,"status":1}`)
				_ = miniredis.Set("auth:token:1", `{"unique_token":"R1bAeiPkJOOA8MqJVgMieGBVd1QcqR65Av7hKpIZTHbe6QrFdzIii6Q7Hsp-YSsEukn7iWRt","time_at":"1594379699"}`)
				miniredis.SetTTL("user:1", time.Second*60*60*24)
				miniredis.SetTTL("auth:token:1", time.Second*60*60*24)
			},
		},
	}
...
func Test_tokenCache_Delete(t *testing.T) {
	client, miniredis := mockredis.Connection()
	...
	tests := []struct {
	...
	}{
		{
			...
			wantErr: miniredis.Exists("auth:token:1"),
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := uc.Delete(tt.args.userID); (err != nil) != tt.wantErr {
				t.Errorf("tokenCache.Delete() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
	miniredis.Close()
